...
[path.mkdir(parents=True, exist_ok=True) for p, path in PATHS.items() if path.exists() is False]
...
# Allow overriding the AUTO_OVERLAY color from config.ini. Expected format: r,g,b,a (0-255)
try:
    if CONFIG.has_section('Openmatb') and 'auto_overlay_color' in CONFIG['Openmatb']:
        raw = CONFIG['Openmatb']['auto_overlay_color']
        parts = [int(p.strip()) for p in raw.split(',')]
        if len(parts) == 4:
            C['AUTO_OVERLAY'] = tuple(parts)
except Exception:
    # If parsing fails, keep the default defined above
    pass

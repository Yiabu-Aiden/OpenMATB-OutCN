...
class AbstractPlugin:
    ...
    def show(self):
        ...
        self._update_automation_overlay()
...
##                self.get_widget('borer').hide()
    ...
    def create_widgets(self):
        ...
        if 'displayautomationstate' in self.parameters.keys():
            if self.parameters['displayautomationstate'] is True:
                overlay_draw_order = self.m_draw + 9
                label_draw_order = overlay_draw_order + 1
                self.add_widget('automode_overlay', Frame, container=self.task_container,
                                fill_color=C['AUTO_OVERLAY'], border_color=C['AUTO_OVERLAY'],
                                draw_order=overlay_draw_order)
                self.add_widget('automode', Simpletext, container=self.task_container,
                                text=str(), color=C['WHITE'], font_size=F['XLARGE'],
                                draw_order=label_draw_order)
    ...
    def _is_automation_active(self):
        return self.parameters.get('automaticsolver') is True


    def _update_automation_overlay(self):
        if self.is_visible() is False:
            return

        if not self.parameters.get('displayautomationstate', False):
            return

        overlay = self.get_widget('automode_overlay')
        label = self.get_widget('automode')
        overlay_active = self._is_automation_active()

        if overlay is not None:
            overlay.set_visibility(overlay_active)

        if label is not None:
            label_text = _('AUTO') if overlay_active else str()

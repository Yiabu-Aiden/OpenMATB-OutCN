...
from core.constants import PATHS as P, COLORS as C, REPLAY_MODE, CONFIG
...
class Communications(AbstractPlugin):
    def __init__(self, label='', taskplacement='bottomleft', taskupdatetime=80):
        ...
        # Allow overriding default voice idiom/gender from config.ini [Communications]
        self.parameters.update(new_par)
        try:
            if CONFIG is not None and CONFIG.has_section('Communications'):
                if 'voiceidiom' in CONFIG['Communications']:
                    self.parameters['voiceidiom'] = CONFIG['Communications']['voiceidiom']
                if 'voicegender' in CONFIG['Communications']:
                    self.parameters['voicegender'] = CONFIG['Communications']['voicegender']
        except Exception:
            # If CONFIG isn't available or something goes wrong, keep the defaults
            pass
